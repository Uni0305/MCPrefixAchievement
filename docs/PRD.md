...
Paper API 1.21.x 기반의 **독립 구동 플러그인**으로, (1) 플레이어 행위 통계를 MySQL에 영속 저장하고, (2) `prefix.yml`에 정의된 달성 조건을 임계값 기반으로 자동 검사하며, (3) 달성 시 보상(아이템/효과/명령어)을 자동 지급하고, (4) 획득한 칭호를 채팅·탭리스트·네임택에 표시·장착할 수 있게 한다.
...
- 미달성 → 달성 전환 시 즉시 보상(아이템/효과/명령어) 자동 지급.
...
│                              ▼                   ▼         │
│                     ┌────────────────┐  ┌────────────────┐ │
│                     │ MySQL (async,  │  │ Reward Disp.   │ │
│                     │ virtual thread)│  │ (item/effect/  │ │
...
**데이터 플로우**: Bukkit 이벤트 → 통계 증가(MySQL) → 달성 검사 → 보상 지급 → (플레이어 선택 시) 칭호 장착 → 3채널 표시.
...
| 칭호 모델 | `model/Prefix.java` | ✅ 구현 (display/stat/value/desc/reward) |
...
#### 3.5.2 보상 자동 지급 (reward 파싱)

현재 `prefix.yml`의 reward는 자유 텍스트("빵 16개", "성급함 효과 5분")로, 자동 지급을 위해 **구조화된 스키마로 마이그레이션**해야 한다.

**제안하는 reward 스키마** (`prefix.yml`)
```yaml
1:
    display-name: "<bold><green>[첫걸음]"
    required-stat: "FIRST_JOIN"
    required-stat-value: 1
    description: "서버에 첫 접속하기"
    reward:
        items:
            - material: BREAD
              amount: 16
...
- 모든 지급은 메인 스레드에서 실행(동기), 달성 검사/DB 기록은 비동기

**호환성**: 기존 자유 텍스트 reward는 description 전용 표시 문자열로 degrade하거나, 마이그레이션 가이드를 제공.
...
#### 3.8.2 prefix.yml (reward 구조화 마이그레이션)

기존 자유 텍스트 reward → 3.5.2의 구조화된 스키마로 마이그레이션. 12종 칭호(`prefix.yml:1-72`) 모두 변환 대상.

**마이그레이션 매핑 예시**
| 칭호 | 기존 reward 텍스트 | 구조화 후 |
|------|-------------------|-----------|
| [첫걸음] | "빵 16개, 횃불 32개" | `items: [BREAD×16, TORCH×32]` |
| [보석 수집가] | "성급함 효과 5분" | `effects: [HASTE, 6000t, amp 1]` |
| [망치 나가신다] | "점프 강화 명령어(쿨타임 1시간, 유지시간 3분)" | `commands: ["jumpboost {player} 3"]` |
...
| **v1.3** | reward 스키마 마이그레이션 + 버퍼링 튜닝 | 운영 안정화 |
...
| 4 | reward 파싱 실패(잘못된 material/effect) | 보상 미지급 | Config 로드 시 검증, 실패 시 칭호 비활성화 + 로그 |
| 5 | 네임택/탭리스트 표시가 타 플러그인과 충돌 | 표시 깨짐 | `display.*` 채널별 on/off, 이벤트 우선순위 조정 |
| 6 | `GET_DRAGON_EGG` 감지 신뢰성 | 달성 누락 | 다중 이벤트(블럭 파괴/아이템 획득) 조합 감지, 보수적 집계 |
| 7 | MySQL 연결 실패 시 플러그인 비활성화 | 전체 기능 정지 | `MCPrefixAchievement.onEnable`의 try-catch 유지, 재시도 로직 검토 |
